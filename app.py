...
def load_and_preprocess_data(file_path_or_buffer):
    ...
    # Standardize column names to prevent KeyError across CSV variations
    clean_cols = []
...
with tab4:
    ...
    for idx, model_name in enumerate(model_keys):
        ...
        final_pred = 0 if functioning_input == "No" else int(round(np.maximum(0, raw_pred)))
...
with tab5:
    ...
    st.markdown("""
    * **Peak Hour Management**: Rush hour demand spikes significantly around **08:00** and peaks at **18:00**. Rebalancing teams should reposition bicycles prior to these hours.
    ...
    * **Seasonal Fleet Capacity**: Summer experiences maximum operational volume, whereas Winter demand drops substantially.
    * **Model Deployment**: **Gradient Boosting Regression** should be integrated into real-time dispatch systems.
    """)
